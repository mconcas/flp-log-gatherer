...
class JournalCollector:
    ...
    async def collect_journal(
        ...
        try:
            ...
            logger.info(f"Collecting journal from {hostname} (app: {app_name}, unit: {unit})")
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    result['success'] = True
                    result['lines_collected'] = len(result['output'].splitlines())
            ...
            result['error'] = str(e)
            logger.error(f"Exception during journal collection from {hostname}: {e}")
    ...
    async def check_journal_available(self, hostname: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Could not check journalctl availability on {hostname}: {e}")
            return False
    ...
    async def list_available_units(self, hostname: str) -> List[str]:
        ...
        try:
            ssh_cmd = self._build_ssh_command(hostname)
            ssh_cmd.append("systemctl list-units --type=service --no-pager --no-legend | awk '{print $1}'")
            ...
            if process.returncode == 0:
                units = stdout.decode('utf-8', errors='replace').strip().split('\n')
